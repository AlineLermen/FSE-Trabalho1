...
int main(int argc, const char * argv[]) {
    ...
    printf("1 (op desativada) - TERMINAL (teclado): ");
    printf("atraves de entrada de teclado no terminal\n\n");
    printf("2 - POTENCIOMETRO: ");
    printf("atraves do potenciometro via UART\n\n");
    printf("3 - CURVA REFLOW: ");
    ...
    switch (escolhaReferencia) {
        case 1:
            printf("Opção desativada do sistema\n");
            encerraPrograma();
            break;
        case 2:
            modoControleS = 0;
    ...
    while(1) {
        ...
        float tempPotenciometro = 0.0;
        float tempExterna = 0.0;

        float tempReferenciaLcd;
        ...
        *p_tx_buffer++ = crcBytes3[1];
        ...
        if ((comandoLido == 3 && modoControleB != 0) || (comandoLido == 4 && modoControleB != 1)) {
            ...
            if (comandoLido == 3) {
                modoControleS = 0;
                modoControleB = 0;
                printf("Modo de controle alterado para: POTENCIOMETRO\n");
            }                
            if (comandoLido == 4) {
                modoControleS = 1;
                modoControleB = 1;
        ...
        if ((comandoLido == 1 && estadoSistemaB != 1) || (comandoLido == 2 && estadoSistemaB != 0)) {
            ...
            if (modoControleB == 0)
                printf("Modo de controle atual do sistema: POTENCIOMETRO\n");
            printf("\n");
            ...
            *p_tx_buffer++ = crcBytes[1];
            ...
            if (modoControleB == 0) {
                ...
                *p_tx_buffer++ = crcBytes[1];
                ...
                if (resultado == 0) {
                    sleep(1);
                    tempPotenciometro = leFloatUART(uart0_filestream);
                } else
                    printf("Erro - Problema na UART.\n");

                sinalCont = rotinaPID(tempPotenciometro, tempInterna);
                tempReferenciaLcd = tempPotenciometro;
            ...
            // Caso CURVA REFLOW (TR)
                ...
                tempReferenciaLcd = tempReferenciaCurva;
            ...
            mostraTempDisplay(modoControleB, tempInterna, tempReferenciaLcd, tempExterna);

            escreveArquivoCSV(tempExterna, tempInterna, tempReferenciaLcd, sinalCont);
            mostraLogs(tempExterna, tempInterna, tempReferenciaLcd, sinalCont);

        } else if (estadoSistemaB == 0) {
            mostraDesligadoDisplay();
